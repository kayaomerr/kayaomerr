<div align="center">

# Ömer KAYA

### Founder & Developer

Building modern mobile applications, backend systems and aviation technologies.

<img src="https://readme-typing-svg.herokuapp.com?font=Inter&weight=500&size=22&pause=1000&color=CEAE69&center=true&vCenter=true&width=700&lines=Computer+Engineering+Student;React+Native+Developer;Full+Stack+Developer;Building+METAR+Alert+✈️;Founder+%26+Developer" />

</div>

---

# About Me

🎓 Computer Engineering student at Kırıkkale University

📱 Developing modern mobile applications with React Native & Expo

⚙️ Building scalable backend systems using Node.js, PostgreSQL and Prisma

✈️ Currently building **METAR Alert**, an aviation weather platform for pilots and aviation students

🚀 Passionate about mobile development, backend architecture and startup culture

---

# Tech Stack

<div align="center">

## Mobile

![React Native](https://img.shields.io/badge/React%20Native-0A0A0A?style=for-the-badge\&logo=react\&logoColor=61DAFB)
![Expo](https://img.shields.io/badge/Expo-0A0A0A?style=for-the-badge\&logo=expo\&logoColor=white)

## Frontend

![Next JS](https://img.shields.io/badge/Next.js-0A0A0A?style=for-the-badge\&logo=next.js\&logoColor=white)
![TypeScript](https://img.shields.io/badge/TypeScript-0A0A0A?style=for-the-badge\&logo=typescript\&logoColor=3178C6)
![TailwindCSS](https://img.shields.io/badge/TailwindCSS-0A0A0A?style=for-the-badge\&logo=tailwind-css\&logoColor=38B2AC)

## Backend

![Node.js](https://img.shields.io/badge/Node.js-0A0A0A?style=for-the-badge\&logo=nodedotjs\&logoColor=339933)
![Express](https://img.shields.io/badge/Express-0A0A0A?style=for-the-badge\&logo=express\&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-0A0A0A?style=for-the-badge\&logo=postgresql\&logoColor=316192)
![Prisma](https://img.shields.io/badge/Prisma-0A0A0A?style=for-the-badge\&logo=prisma\&logoColor=white)

## Cloud & DevOps

![Railway](https://img.shields.io/badge/Railway-0A0A0A?style=for-the-badge\&logo=railway\&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-0A0A0A?style=for-the-badge\&logo=docker\&logoColor=2496ED)
![Git](https://img.shields.io/badge/Git-0A0A0A?style=for-the-badge\&logo=git\&logoColor=F05032)

## Apple Ecosystem

![Apple](https://img.shields.io/badge/Apple-0A0A0A?style=for-the-badge\&logo=apple\&logoColor=white)
![Xcode](https://img.shields.io/badge/Xcode-0A0A0A?style=for-the-badge\&logo=xcode\&logoColor=147EFB)
![TestFlight](https://img.shields.io/badge/TestFlight-0A0A0A?style=for-the-badge\&logo=apple\&logoColor=white)

</div>

---

# Featured Project

## ✈️ METAR Alert

Modern aviation weather application built for pilots and aviation students.

### Features

* Real-time METAR data
* Smart weather alerts
* Push notification system
* Airport-based subscription model
* Aviation-focused modern UI
* Real-time METAR decoding logic
* Flight condition evaluation system

### Infrastructure

* React Native + Expo
* Node.js + Express
* PostgreSQL + Prisma
* Railway Deployment
* RevenueCat Subscriptions
* Apple Sign In
* APNs Push Notifications

---

# GitHub Stats

<div align="center">

<img height="180em" src="https://github-readme-stats.vercel.app/api?username=kayaomerr&show_icons=true&theme=tokyonight&hide_border=true&bg_color=000000"/>

<img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=kayaomerr&layout=compact&theme=tokyonight&hide_border=true&bg_color=000000"/>

</div>

---

# GitHub Streak

<div align="center">

<img src="https://streak-stats.demolab.com?user=kayaomerr&theme=tokyonight&hide_border=true&background=000000"/>

</div>

---

# Contact

📫 [komer6088@gmail.com](mailto:komer6088@gmail.com)

---

<div align="center">

### "Building products, not just projects."

</div>
